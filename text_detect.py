# For now, we will detect whether the text from the user gives a positive feeling or negative feeling by classifying the text as positive, negative, or neutral
import time
import pandas as pd
import numpy as np
...
class analysis_text():
    ...
    def setResult(self, type, res):
        if (type == "neg"):
            self.negativeLabel.configure(
                text="you typed a negative comment : " + str(res + " % \n"))
        elif (type == "pos"):
            self.positiveLabel.configure(
                text="you typed a positive comment :" + str(res) + " % \n")
    ...
    def runAnalysis(self):
        ...
        for sentence in sentences:
            ...
            if ss['compound'] >= 0.05:
                self.normalLabel.configure(
                    text=" you typed a negative statement: ")

            elif ss['compound'] <= 0.05:
                self.normalLabel.configure(
                    text=" you typed negative statement")

            else:
                self.normalLabel.configure(
                    text=" you normal typed statement: ")
